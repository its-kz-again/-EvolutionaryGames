...
class Player():
    ...
    def think(self, mode, box_lists, agent_position, velocity):
        ...
        tmp = []
        if len(box_lists) > 0:
            tmp.append([(box_lists[0].x - agent_position[0]) / CONFIG.get("WIDTH")])
            tmp.append([(box_lists[0].gap_mid - agent_position[1]) / CONFIG.get("HEIGHT")])
        else:
            tmp.append([random.uniform(0.9, 1)])
            tmp.append([random.uniform(0, 0.2)])
        if len(box_lists) > 1:
            tmp.append([(box_lists[1].x - agent_position[0]) / CONFIG.get("WIDTH")])
            tmp.append([(box_lists[1].gap_mid - agent_position[1]) / CONFIG.get("HEIGHT")])
        else:
            tmp.append([random.uniform(0.9, 1)])
            tmp.append([random.uniform(0, 0.2)])
        # if len(box_lists) > 2:
        #     tmp.append([(box_lists[2].x - agent_position[0]) / CONFIG.get("WIDTH")])
        #     tmp.append([(box_lists[2].gap_mid - agent_position[1]) / CONFIG.get("HEIGHT")])
        # else:
        #     tmp.append([[random.uniform(0.9, 1)]])
        #     tmp.append([[random.uniform(0, 0.2)]])

        tmp.append([velocity / 20])

        ans = self.nn.forward(tmp)
        ...
        # x_input = [velocity, agent_position[0], agent_position[1], xs, ys, mode_num]
        # maxi = max(x_input)
        # nn_input = np.array([[velocity / maxi], [agent_position[0] / maxi], [agent_position[1] / maxi], [xs / maxi],
        #                      [ys / maxi], [mode_num / maxi]])
        #
        # ans = self.nn.forward(nn_input)
        #
        # if ans[0][0] > 0.5:
        #     direction = 1
        # else:
